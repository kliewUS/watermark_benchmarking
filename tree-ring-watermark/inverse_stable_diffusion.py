...
# Alias it so the rest of the script doesn't break
...
class InversableStableDiffusionPipeline(ModifiedStableDiffusionPipeline):
    def __init__(self,
        ...
        # 1. Skip the broken StableDiffusionPipeline __init__ 
        # and go to the very base of the library
        from diffusers import DiffusionPipeline
        super(DiffusionPipeline, self).__init__()

        # 2. Manually register ONLY the neural modules
        # This keeps the 'bool' away from the module inspector
        ...
        # 3. Manually attach the boolean and the modern image_processor
        self.requires_safety_checker = requires_safety_checker
        self.image_processor = feature_extractor
        # Add this line to satisfy modern Diffusers requirements
        self.vae_scale_factor = 8
        
        # Also fix the deprecation warning while you are at it
        # Change: num_channels_latents = self.unet.in_channels
        # To:
        self.num_channels_latents = self.unet.config.in_channels
        
        # 4. Modern Diffusers 0.37+ metadata check
        if not hasattr(self, "config"):
            self._internal_dict = {"requires_safety_checker": requires_safety_checker}
