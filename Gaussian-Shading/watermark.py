...
class Gaussian_Shading_chacha:
    ...
    def truncSampling(self, message):
        ...
        return z.to('mps', dtype=torch.float32)

    def create_watermark_and_return_w(self):
        self.watermark = torch.randint(0, 2, [1, 4 // self.ch, 64 // self.hw, 64 // self.hw]).to('mps', dtype=torch.float32)
    ...
    def stream_key_decrypt(self, reversed_m):
        ...
        return sd_tensor.to('mps', dtype=torch.float32)
...
class Gaussian_Shading:
    ...
    def create_watermark_and_return_w(self):
        self.key = torch.randint(0, 2, [1, 4, 64, 64]).to('mps', dtype=torch.float32)
        self.watermark = torch.randint(0, 2, [1, 4 // self.ch, 64 // self.hw, 64 // self.hw]).to('mps', dtype=torch.float32)
        sd = self.watermark.repeat(1,self.ch,self.hw,self.hw)
